        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {roles.map((role) => {
            ...
            return (
              <div
                key={role.id}
                className={`bg-white rounded-xl ${style.border} p-6 shadow-sm flex flex-col transition-all`}
              >
      ...
      <section className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                {roles.map((r) => (
                  <th key={r.id} className="px-8 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">
                    {r.label || r.name}
                  </th>
                ))}
              </tr>
        ...
        <div className="px-8 py-4 bg-slate-50 border-t border-slate-100 flex justify-end">
          <button
            ...
            className="bg-brand text-white px-5 py-2 rounded-xl font-bold text-xs hover:bg-brand-dark transition-all shadow-sm disabled:opacity-50"
          >
